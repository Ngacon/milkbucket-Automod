...
function createAutomodService({ automodRepo, guildSettingsRepo, i18n, redis, logger }) {
  async function checkMessage({ message }) {
    ...
    if (settings.antidup && redis) {
      ...
      // Use a Redis pipeline to fetch the last message and set the new one in a single round-trip.
      const pipeline = redis.pipeline();
      pipeline.get(key);
      pipeline.set(key, content, 'EX', 30);
      const results = await pipeline.exec();
      // results is an array of [error, result] tuples.
      const lastMessage = results && results[0] ? results[0][1] : null;
