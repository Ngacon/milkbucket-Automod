...
class AutomodRepository {
  ...
  buildWhitelistCacheKey(guildId) {
    return `automod-whitelist:${guildId}`;
  }
  ...
  async getThresholds(guildId) {
    ...
    const result = await this.pool.query(
      `
        SELECT warns, action, duration, message
  ...
  async upsertThreshold(guildId, warns, action, duration, message = null) {
    await this.pool.query(
      `
        INSERT INTO automod_thresholds (guild_id, warns, action, duration, message)
        VALUES ($1, $2, $3, $4, $5)
        ...
        DO UPDATE SET
          ...
          updated_at = NOW()
      `,
      [guildId, warns, action, duration == null ? null : duration, message]
    );
  ...
  async addWhitelist(guildId, userId, addedBy) {
    await this.pool.query(
      `
        INSERT INTO automod_whitelist (guild_id, user_id, added_by)
        VALUES ($1, $2, $3)
  ...
  async listWhitelist(guildId) {
    const cacheKey = this.buildWhitelistCacheKey(guildId);
    const cached = await this.readJsonCache(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    await this.writeJsonCache(cacheKey, result.rows);
    return result.rows;
  }

  async isWhitelisted(guildId, userId) {
    const rows = await this.listWhitelist(guildId);
    return rows.some((row) => row.user_id === String(userId));
  }
}
