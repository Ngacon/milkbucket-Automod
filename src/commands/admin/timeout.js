...
module.exports = {
  meta: {
    ...
    cooldown: 2,
    args: { min: 2, max: 20, usage: 'timeout @user [time] [reason]' },
  ...
  async execute({ message, args, t, respond, repos }) {
    ...
    if (!member || !duration) {
      await respond({
        ...
        description: t('admin.responses.invalidTimeoutInput')
      });
      return;
    }
    ...
    await respond({
      color: EMBED_COLORS.WARNING,
      title: t('admin.responses.timeoutTitle'),
      author: {
        name: member.user.tag,
        iconURL: member.user.displayAvatarURL({ size: 128 })
      },
      thumbnail: member.user.displayAvatarURL({ size: 256 }),
      description: t('admin.responses.timeoutApplied', {
